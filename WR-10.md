...
The Deformation Monitoring of Critical Infrastructure (DMCI) Capability is the service, which provides infrastructure operators with such highly accurate deformation data mapped to road network geometries. Within the framework of the ESA - Green Transition Information Factories project the potentials of this capability are being publicly demonstrated on a corridor of approximately 60 km length at one of the main Austrian motorways the A14, however, it can be rolled out to the entire road or railway network of a city, region our country.
...
* the classification of the point and its location along the road in km,
...
* the time series for this point (combining in case of EGMS data also different processing epochs).
...
The points are color-coded, with white points showing no significant deformations, red and yellow points showing subsidence and blue points highlighting uplifts. The reliability of the point, as expressed by the number of available persistent scatterers in the segment, is expressed by the diameter of the cirlce used as a marker. No circle is displayed if no reliable data is available due to insignificant number of reflection points for the radar signal or unacceptably high scatter in the readings.
